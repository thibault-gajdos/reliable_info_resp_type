rm(list=ls(all=TRUE))  ## efface les données


library(tidyverse)
library(cmdstanr)
library(simcausal)
library(posterior)
library(bayesplot)

######################################
## prepare the data for fits etc.
#####################################

exp <- 2; # specify which experiment to run
data <- read.csv(paste0("../data/DATA_EXPChoiceReliab_Exp", as.character(exp),".csv"))
data <- data %>%
  separate(Sample_Reliability, into = paste0("proba_", 1:6), sep = "\\s+") %>%
  mutate(across(starts_with("proba_"), as.numeric)) %>%     
  mutate(color = str_extract_all(Sample_Color, "blue|red"))  %>%
  unnest_wider(color, names_sep = "_") %>% 
  rowwise() %>%
  mutate(sample_number = sum(!is.na(c_across(starts_with("proba_")))))
data <- data %>%
  mutate_at(vars(starts_with("color")), ~ ifelse(. == "blue", 1, 2))


# filter the first row
dat <- data %>% 
  group_by(ParticipantPrivateID) %>%
  slice(1) %>% slice(rep(1,50))
data_duplicate <- bind_rows(data,dat)

# choose rows 50 times randomly
dat <- data %>%
   group_by(ParticipantPrivateID) %>%
   slice_sample(n=50)
data_replacement <- bind_rows(data,dat)


# define the data set again 
data = data_duplicate

data = data_replacement

# prepare the data list
N = length(unique(data$ParticipantPrivateID))
T_max = data%>% group_by(ParticipantPrivateID) %>% summarise(n=n())
T_max = T_max$n[1]
I =  sum(grepl("color", names(data))) ## number of samples/trial
## compute trials by subject
d <- data %>%
  group_by(ParticipantPrivateID) %>%
  summarise(t_subjs = n())
t_subjs <- d$t_subjs
subjs <- unique(data$ParticipantPrivateID)
Tsubj = t_subjs

## Initialize data arrays
color <- array( -1, c(N, T_max, I))
proba <- array(-1, c(N, T_max, I))
sample <- array(-1, c(N, T_max))

## fill the  arrays
for (n in 1:N) {
  t <- t_subjs[n] ## number of trials for subj i
  data_subj <- data %>% filter(ParticipantPrivateID == subjs[n])
  for (j in 1:t) {
    data_subj_t <- data_subj[j,]
    sample[n,j] <- data_subj_t$sample_number    
    for (i in 1:I) {
      color_var <- paste0("color_", i)
      proba_var <- paste0("proba_", i)
      color[n, j, i] <- data_subj[[color_var]][j]
      proba[n, j, i] <- data_subj[[proba_var]][j]/100
    }
  }
}


## ####################################################
## ## SIMULATE RESPONSES
## ###################################################
simul_trunc_simplified <- function(alpha, beta, lambda, theta, psi, l_obs, color_info, proba_info){
  ## initialisation
  T = nrow(color_info)
  I = ncol(color_info)
  choice  =rep(-1,T)
  evidence_1  =rep(-1,T)
  evidence_2  =rep(-1,T)
  ## generate responses
  for (t in c(1:T)){
    evidence = c(0,0) ## initial value
    for (i in c(1:I)){
      l = log(proba_info[t,i] / (1 - proba_info[t,i]) ) ; 
      log_odds = alpha * l * (psi / l_obs) + (1-alpha)* beta; 
      if (color_info[t,i] == 1){## blue
        evidence[1] = evidence[1] + exp(lambda * (i - I)) * log_odds;    
      }else{## red 
        evidence[2] = evidence[2] + exp(lambda * (i - I)) * log_odds;
      }
    }
    x <- plogis(theta*(evidence[2] - evidence[1]))
    choice[t] <- rbern(1, x) + 1 # 2 = red, 1 = blue
    evidence_1[t] = evidence[1]
    evidence_2[t] = evidence[2]
  }
  return(list(choice,evidence_1,evidence_2))
}

l_obs = log(max(proba) / (1 - max(proba)))
mu_alpha = 0.8;
mu_beta = 0.25;
mu_lambda = 0.1;
mu_theta = 3;
mu_psi = 3;
sigma_alpha = 0.2
sigma_beta = 0.2
sigma_lambda = 0.02
sigma_theta = 0.5
sigma_psi = 0.5

mu_alpha = 1;
mu_beta = 0.05;
mu_lambda = 0.1;
mu_theta = 1;
mu_psi = 1;
sigma_alpha = 0.001
sigma_beta = 0.001
sigma_lambda = 0.001
sigma_theta = 0
sigma_psi = 0.001

## ## SIMULATION
choice  <- array(-1, c(N, T_max))
params_indiv  <- array(-1, c(N, 5))
for (n in 1:N){
  params_indiv[n,1] = rnorm(1,mu_alpha,sigma_alpha)
  params_indiv[n,2] = rnorm(1,mu_beta,sigma_beta)
  params_indiv[n,3] = rnorm(1,mu_lambda,sigma_lambda)
  params_indiv[n,4] = rnorm(1,mu_theta,sigma_theta)
  params_indiv[n,5] = rnorm(1,mu_psi,sigma_psi)
  choice[n,] <-  simul_trunc_simplified(alpha = params_indiv[n,1], beta = params_indiv[n,2], lambda = params_indiv[n,3], theta = params_indiv[n,4], psi = params_indiv[n,5], l_obs, color_info = color[n,,], proba_info = proba[n,,])[[1]]
}


###### fit the model ########
I_max = I
data_list <- list(
  N = N,
  T_max = T_max,
  I_max = I_max,
  Tsubj = t_subjs,
  color = color,
  proba = proba,
  choice = choice, ## 
  sample = sample,
  l_obs = l_obs
)
save(data_list, file = paste0('../results/fits/simulation/data_list', '.rdata'))

whichmodel <- 'choice_log_trunc_simplified';


data_list$grainsize = 5 ## specify grainsize for within chain parallelization

## Compile the model
model <- cmdstan_model(
  stan_file = paste0('../stan/', whichmodel, '.stan'),
  force_recompile = TRUE, ## necessary if you change the mode
  cpp_options = list(stan_opencl = FALSE, stan_threads = TRUE), ## within chain parallel
  stanc_options = list("O1"), ## fastest sampling
  compile_model_methods = TRUE ## necessary for loo moment matching
)


## Sampling
fit <- model$sample(
  data = data_list,
  seed = 4321,
  chains = 4,
  parallel_chains = 4,
  threads_per_chain = 4,
  iter_warmup = 2000,
  iter_sampling = 1000,
  max_treedepth = 12,
  adapt_delta = .9,
  save_warmup = FALSE
)


## Save results
save(fit, file = paste0('../results/fits/simulation', '/fit_',whichmodel, '.rdata'))



## LOAD MODELS FIT
load(paste0('../results/fits/simulation','/fit_', whichmodel, '_repetition', '.rdata'))


## Pairs plots
# Extract the current fit
posterior_samples <- fit$draws()
posterior_df <- as_draws_df(posterior_samples)
selected_params <- posterior_df[, grepl("^mu", colnames(posterior_df)) & !grepl("^mu_pr", colnames(posterior_df))]
plot <- mcmc_pairs(selected_params,
                   off_diag_args = list(size = 0.1, alpha=0.1))
plot_file <- paste0('../results/plots/exp',exp, '/',target,'/pairs_plot_', whichmodel,"_exp", exp)
ggsave(plot, file = paste0(plot_file, ".jpeg"), width = 8, height = 8)


## Traces
selected_params <- posterior_df[, grepl("^mu", colnames(posterior_df))]
plot <- mcmc_trace(selected_params, size=0.1)
plot_file <- paste0('../results/plots/exp',exp, '/',target,'/trace_plot_', whichmodel,"_exp", exp)
ggsave(plot, file = paste0(plot_file, ".jpeg"), width = 10, height = 8)


# ---------------------------------------------------
# CHECK MODEL PREDICTION 
# ---------------------------------------------------

proba <- posterior_df[, grepl("^pred_proba", colnames(posterior_df)) ]
median <- sapply(proba, function(var) median(var))
dat = c(); sub = c(); tr = c(); j = 0; 
for (t in 1:data_list$T_max){
  for (s in 1:data_list$N){
    j = j + 1; 
    sub[j] = s;
    tr[j] = t;
    dat[j] = 2-choice[s,t]; ## 0 = red, 1 = blue
  }
}
d <- c(); 
d <- data.frame(median = median, data = dat, subj=sub, trial=tr) 
cor(d$median, d$data)

# Create a single plot with all subjects
plot <- ggplot(d, aes(x = factor(data), y = median, color = factor(subj))) +
  geom_violin() +       # Function curve
  facet_wrap(~ subj, ncol = 8) + # Create a plot for each subject
  theme_minimal() +
  labs(title = "Predictive check by Subject", x = 'choice', y = 'prediction') +
  theme(legend.position = "none")



