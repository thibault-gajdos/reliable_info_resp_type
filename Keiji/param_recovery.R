...
for (t in 1:data_list$T_max){
  for (s in 1:data_list$N){
    ...
    sub[j] = s;
    tr[j] = t;
    dat[j] = 2-choice[s,t]; ## 0 = red, 1 = blue
  }
}
d <- c(); 
...
plot <- ggplot(d, aes(x = factor(data), y = median, color = factor(subj))) +
  ...
  theme_minimal() +
  labs(title = "Predictive check by Subject", x = 'choice', y = 'prediction') +
