functions {
  ...
  real partial_sum(array[] int slice_indices,
                 ...
                 array[,,] int color,
                 array[,,] real proba,
                 array[,] real lo_proba, // log odds of probability estimates (<=0: a higher likelihood in favour of red; >0: a higher likelihood in favour of blue)
                 matrix param_raw,
                 real l_obs){
    real lp = 0;
    for (i in 1:size(slice_indices)) {
      ...
      for (t in 1:Tsubj[n]) {
        ...
        real lp_raw = normal_lpdf(lo_proba[n,t] | evidence_safe[1]-evidence_safe[2],  clamp(params[4], 0.0001));
  ...
  real compute_log_lik(int sample_size, array[] int color_data, array[] real proba_data, 
                       real lo_proba, real alpha, real beta, real lambda, real sigma_est, real psi, real l_obs) {
    vector[2] evidence = compute_evidence(sample_size, color_data, proba_data, alpha, beta, lambda, psi, l_obs);
    vector[2] evidence_safe = clamp_vector(evidence, -100, 100);
    real lp_raw = normal_lpdf(lo_proba | evidence_safe[1]-evidence_safe[2],  clamp(sigma_est, 0.0001));
    real lp_safe = clamp(lp_raw, -1000);
...
model {
 ...
 target += reduce_sum(partial_sum, indices, grainsize,
                     mu_pr, sigma_pr, 
                     Tsubj, sample, color, proba, lo_proba, param_raw, l_obs);
}
